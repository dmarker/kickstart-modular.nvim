...
macos (TODO figure these out ...):
```
brew tap mesca/luarocks
brew install gopls lua-language-server luarocks51 pynvim stylua superhtml \
...
I can't seem to convince neovim to use gmake for MAKE (actually its probably lazy I need to
convince). But it does honor environment so for the first run and whenever you want to update
modules you need to:
```
MAKE=/usr/local/bin/gmake nvim
```

Actually I think I found where I need to change for a couple plugins. Always worth looking:
```
find lua -type f -exec grep "'make'" {} \; -print
```
